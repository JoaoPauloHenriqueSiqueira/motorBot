...
// const company = "1";
// const url = "http://localhost/orc2/public/api/admin/";
// const userSale = "1"
...
var shoppingCart = (function () {
  ...
  function Item(product, count) {
    ...
    this.valor_moeda = product.valor_moeda;
    this.photo = product.photos;
    this.control_quantity = product.control_quantity;
  ...
  obj.totalCart = function () {
    var totalCart = 0;
    for (var item in cart) {
      totalCart += cart[item].valor * cart[item].count;
    }
...
function addCart(element) {
  ...
  if (typeof product.photos[0] !== 'undefined') {
    photo = product.photos[0];
  }
...
function displayCart() {
  ...
  for (var i in cartArray) {
    let quant = "";
    if (cartArray[i].control_quantity > 0) {
      quant = `Unidade(s) disponível(is): ${cartArray[i].quantity}<br>`;
    }

    let sizes = cartArray[i].sizes;
    let optionsSize = "";
    if (sizes.length > 0) {
      optionsSize += "<option disabled selected>Selecione o tamanho</option>";
      sizes.forEach((element) => {
        optionsSize += `<option value="${element.id}">${element.name}</option>`;
      });
    ...
    <div class="media">
      <div class="img-holder ml-1 mr-2">
          <a href="#"><img src="${cartArray[i].photo[0]}"
      ...
      <div class="media-body mt-auto mb-auto">
          ...
          <label for="size${cartArray[i].id}" >Tamanho:</label>
            <select id="size${cartArray[i].id}">
            ${optionsSize}
            </select>
          </label>
